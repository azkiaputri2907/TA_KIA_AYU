<aside :class="sideBarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="flex-1 px-4 space-y-2 overflow-y-auto">
        <a href="{{ route('admin.dashboard') }}" 
           class="flex items-center gap-3 py-3 px-4 rounded-xl {{ Request::is('admin') ? 'bg-white text-[#a044ff] font-bold shadow-lg' : 'opacity-70 hover:opacity-100 hover:bg-white/5 transition' }}">
            <i class="fas fa-th-large w-5"></i> <span>Dashboard</span>
        </a>
        
        <a href="{{ route('visits.index') }}" 
        class="flex items-center gap-3 py-3 px-4 rounded-xl {{ Request::routeIs('visits*') ? 'bg-white text-[#a044ff] font-bold shadow-lg' : 'opacity-70 hover:opacity-100 hover:bg-white/5 transition' }}">
            <i class="fas fa-file-alt w-5"></i> <span>Data Kunjungan</span>
        </a>
        <a href="{{ route('admin.survey') }}" 
           class="flex items-center gap-3 py-3 px-4 rounded-xl {{ Request::is('admin/survey*') ? 'bg-white text-[#a044ff] font-bold shadow-lg' : 'opacity-70 hover:opacity-100 hover:bg-white/5 transition' }}">
            <i class="fas fa-poll w-5"></i> <span>Data Survey</span>
        </a>

        <a href="{{ route('admin.laporan') }}" 
        class="flex items-center gap-3 py-3 px-4 rounded-xl transition {{ request()->routeIs('admin.laporan') ? 'bg-white text-[#a044ff] font-bold shadow-lg' : 'opacity-70 hover:opacity-100 hover:bg-white/5 transition' }}">
