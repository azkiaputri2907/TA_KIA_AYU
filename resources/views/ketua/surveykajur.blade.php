...
<div class="flex h-screen bg-[#f8f9fd] overflow-hidden" x-data="{ sideBarOpen: false }">
    ...
    @include('components.sidebarkajur') {{-- Pastikan menggunakan sidebar khusus kajur --}}
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div class="md:hidden flex items-center justify-between p-4 bg-white shadow-sm z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-gray-500 font-medium">Laporan kepuasan tamu</p>
                </div>
            </div>
        ...
        <div class="flex-1 p-4 md:p-10 overflow-y-auto custom-scrollbar">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-2xl md:text-3xl font-extrabold text-gray-800 tracking-tight hidden md:block">
                            Laporan <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#ff3366] to-[#a044ff]">Survey</span>
                        </h2>
                        <p class="text-gray-500 text-xs md:text-sm mt-1 hidden md:block">Monitoring kepuasan pelayanan Jurusan</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-2xl md:rounded-[2rem] shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row gap-3 md:gap-4">
                <form action="{{ route('ketua.survey.index') }}" method="GET" class="relative flex-1 w-full">
                ...
                <div class="w-full md:w-auto">
                    <select onchange="window.location.href=this.value" class="w-full md:w-72 bg-gray-50 px-4 py-3 rounded-xl text-gray-600 font-bold outline-none cursor-pointer hover:bg-gray-100 transition text-sm border-none appearance-none">
                        <option value="{{ route('ketua.survey.index') }}">Semua Prodi</option>
                        @foreach(['D3 Teknik Listrik', 'D3 Teknik Elektronika', 'D3 Teknik Informatika', 'D4 Teknologi Rekayasa Pembangkit Energi', 'D4 Sistem Informasi Kota Cerdas', 'Lainnya'] as $prodi)
                            <option value="{{ route('ketua.survey.index', ['prodi' => $prodi]) }}" {{ request('prodi') == $prodi ? 'selected' : '' }}>
                                {{ $prodi }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr class="bg-gradient-to-r from-[#ff3366] via-[#a044ff] to-[#3366ff] text-white">
                                ...
                                <th class="px-4 py-4 md:px-6 md:py-5 text-[10px] md:text-xs font-bold uppercase tracking-wider text-center">Detail</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-purple-50/30 transition duration-200">
                                ...
                                <td class="px-4 py-3 md:px-6 md:py-4 text-center">
                                    <button onclick='showDetailSurvey(@json($s), @json($s->visit), {{ $avg }})' class="mx-auto w-10 h-10 rounded-xl flex items-center justify-center bg-blue-50 text-blue-500 hover:bg-blue-500 hover:text-white transition-all shadow-sm">
...
{{-- MODAL DETAIL (Sama Seperti Admin) --}}
    ...
    <div class="bg-white w-full md:max-w-md rounded-2xl overflow-hidden shadow-2xl transform scale-95 transition-transform duration-300" id="modalDetailContent">
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="bg-gray-50 p-3 rounded-xl"><p class="text-[10px] text-gray-400 font-bold uppercase">Kecepatan</p><p class="text-sm font-bold text-gray-700" id="detailKecepatan">0/5</p></div>
                <div class="bg-gray-50 p-3 rounded-xl"><p class="text-[10px] text-gray-400 font-bold uppercase">Keramahan</p><p class="text-sm font-bold text-gray-700" id="detailKeramahan">0/5</p></div>
                <div class="bg-gray-50 p-3 rounded-xl"><p class="text-[10px] text-gray-400 font-bold uppercase">Kejelasan</p><p class="text-sm font-bold text-gray-700" id="detailKejelasan">0/5</p></div>
                <div class="bg-gray-50 p-3 rounded-xl"><p class="text-[10px] text-gray-400 font-bold uppercase">Kenyamanan</p><p class="text-sm font-bold text-gray-700" id="detailKenyamanan">0/5</p></div>
            </div>
